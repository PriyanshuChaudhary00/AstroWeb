    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-primary text-primary-foreground py-16">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <h1 className="font-serif text-4xl md:text-5xl font-bold mb-4" data-testid="text-horoscope-title">
            Daily Horoscope
          </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {zodiacSigns.map((sign) => (
            <Link key={sign.slug} href={`/horoscope/${sign.slug}`}>
              <Card className="hover-elevate active-elevate-2 transition-all duration-300 cursor-pointer group" data-testid={`card-zodiac-${sign.slug}`}>
                <CardContent className="p-6 text-center space-y-3">
                  <div className="text-5xl text-accent group-hover:scale-110 transition-transform">
                    {sign.symbol}
                  </div>
                  <div>
                    <h3 className="font-serif text-xl font-bold mb-1" data-testid={`text-sign-${sign.slug}`}>
                      {sign.name}
                    </h3>
